...
var createRemoteStateStorage = require('../../remoteStateStorage.js')
var rlp = require('rlp')
...
createRemoteStateStorage(function (err, remoteStateStorage) {
  if (err) {
    ...
    var vm = new VM({state: remoteStateStorage})
    ...
    function setup (cb) {
      ...
      // store in the trie
      remoteStateStorage.put(address, account.serialize(), cb)
    }
    ...
    function checkResults (cb) {
      ...
      remoteStateStorage.get(createdAddress, function (err, val) {
    ...
    function readStorage (cb) {
      ...
      var storageTrie = remoteStateStorage.copy()

      // Since we are using a copy we won't change the
      // root of `remoteStateStorage`
