...
module.exports = function runBlockchainTest (options, testData, t, cb) {
  const createRemoteStateStorage = (function() {
    if (options.dist) {
      return require('../dist/remoteStateStorage.js')
    } else {
      return require('../lib/remoteStateStorage.js')
    }
  })()
  var blockchainDB = levelMem()
  createRemoteStateStorage(function (err, state) {
    if (err) {
      console.log("Could not create state!")
      t.end()
    } else {
      var validate = false
      ...
      async.series([
        ...
        function (done) {
          ...
          state.getRoot(function (err, stateRoot) {
            if (err) {
              done(err)
            }

            t.equal(stateRoot.toString('hex'), genesisBlock.header.stateRoot.toString('hex'), 'correct pre stateRoot')
        ...
        function getHead (done) {
          vm.blockchain.getHead(function (err, block) {
            ...
            if (!options.debug) {
              ...
              state.setRoot(block.header.stateRoot)
            }
            done(err)
          })
        },
