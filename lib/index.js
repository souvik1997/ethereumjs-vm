...
VM.deps = {
  ethUtil: ethUtil,
  Account: require('ethereumjs-account').default,
 ...
 * @param {RemoteStateStorage} opts.state a RemoteStateStorage instance for the state tree (ignored if stateManager is passed)
...
function VM (opts = {}) {
  ...
  if (opts.stateManager) {
    ...
    var remoteStateStorage = opts.state
    if (opts.activatePrecompiles) {
      for (var i = 1; i <= 8; i++) {
        remoteStateStorage.put(new BN(i).toArrayLike(Buffer, 'be', 20), new Account().serialize())
      }
    }
    this.stateManager = new StateManager({ remoteStateStorage, common: this._common })
  }
...
VM.prototype.copy = function (cb) {
  this.stateManager.copy(function (err, newInstance) {
    if (err) {
      cb(err, null)
    } else {
      cb(null, new VM({ stateManager: newInstance, blockchain: this.blockchain }))
    }
  }.bind(this))
}
