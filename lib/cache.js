...
module.exports = class Cache {
  constructor (remoteStateStorage) {
    this._cache = Tree()
    this._checkpoints = []
    this._remoteStateStorage = remoteStateStorage
  }
   ...
   * @param {bool} [fromRemoteStateStorage]
   */
  put (key, val, fromRemoteStateStorage = false) {
    const modified = !fromRemoteStateStorage
   ...
   * in the underlying remoteStateStorage.
   * @param {Buffer} key - Address of account
   * @param {Function} cb - Callback with params (err, account)
   */
  getOrLoad (key, cb) {
  ...
  /**
   * Warms cache by loading their respective account from remoteStateStorage
  ...
  flush (cb) {
    ...
    async.whilst(() => next, (done) => {
      if (it.value && it.value.modified) {
        ...
        this._remoteStateStorage.put(Buffer.from(it.key, 'hex'), it.value.val, () => {
          next = it.hasNext
          it.next()
          done()
        })
        ...
        this._remoteStateStorage.del(Buffer.from(it.key, 'hex'), () => {
          next = it.hasNext
          it.next()
          done()
        })
  ...
  /**
   * Revert changes to cache last checkpoint (no effect on remoteStateStorage).
   */
  revert () {
  ...
  /**
   * Commits to current state of cache (no effect on remoteStateStorage).
   */
  commit () {
