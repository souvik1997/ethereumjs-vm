...
const RemoteStateStorage = require('./remoteStateStorage.js')
...
function StateManager (opts = {}) {
  ...
  self._remoteStateStorage = opts.remoteStateStorage
  self._storageTries = {} // the storage trie cache
  self._cache = new Cache(self._remoteStateStorage)
 ...
 * @method copy
 */
proto.copy = function (cb) {
  this._remoteStateStorage.copy(function (err, newInstance) {
    if (err) {
      cb(err, null)
    } else {
      cb(null, new StateManager({ remoteStateStorage: newInstance }))
    }
  })
}
...
proto.putAccount = function (address, account, cb) {
  ...
  // self._remoteStateStorage.put(addressHex, account.serialize(), cb)
  cb()
}
...
proto.putContractCode = function (address, value, cb) {
  ...
  self.getAccount(address, function (err, account) {
    ...
    account.setCode(self._remoteStateStorage, value, function (err) {
      if (err) {
        return cb(err)
      }
...
proto.getContractCode = function (address, cb) {
  ...
  self.getAccount(address, function (err, account) {
    ...
    account.getCode(self._remoteStateStorage, cb)
  })
}
...
proto._lookupStorageTrie = function (address, cb) {
  ...
  self.getAccount(address, function (err, account) {
    ...
    self._remoteStateStorage.copy(function (err, newInstance) {
      if (err) {
        cb(err, null)
      } else {
        newInstance.setRoot(account.stateRoot)
        newInstance._checkpoints = []
        cb(null, newInstance)
      }
    })
  })
...
proto._modifyContractStorage = function (address, modifyTrie, cb) {
  ...
  self._getStorageTrie(address, function (err, storageTrie) {
    ...
    function finalize (err) {
      ...
      storageTrie.getRoot(function (err, stateRoot) {
        if (err) {
          cb(err)
        } else {
          contract.stateRoot = stateRoot
...
proto.checkpoint = function (cb) {
  var self = this
  self._remoteStateStorage.checkpoint(function (err, checkpointedInstance) {
    if (err) {
      cb(err, null)
    } else {
...
proto.commit = function (cb) {
  ...
  self._remoteStateStorage.commit(function () {
...
proto.revert = function (cb) {
  var self = this
  // setup trie checkpointing
  self._remoteStateStorage.revert(function (err, revertedInstance) {
    if (err) {
      cb(err, null)
    } else {
      // setup cache checkpointing
...
proto.getStateRoot = function (cb) {
  ...
  self._cache.flush(function (err) {
    ...
    self._remoteStateStorage.getRoot(function (err, stateRoot) {
      if (err) {
        cb(err, null)
      } else {
        cb(null, stateRoot)
...
proto.setStateRoot = function (stateRoot, cb) {
  ...
  self._cache.flush(function (err) {
    if (err) { return cb(err) }
    if (stateRoot === self._remoteStateStorage.EMPTY_TRIE_ROOT) {
      self._remoteStateStorage.setRoot(stateRoot)
      self._cache.clear()
      return cb()
    }
    self._remoteStateStorage.checkRoot(stateRoot, function (err, hasRoot) {
      if (err || !hasRoot) {
...
proto.hasGenesisState = function (cb) {
  ...
  this._remoteStateStorage.checkRoot(root, cb)
}
...
proto.generateGenesis = function (initState, cb) {
  ...
  async.eachSeries(addresses, function (address, done) {
    ...
    self._remoteStateStorage.put(address, account.serialize(), done)
  }, cb)
}
